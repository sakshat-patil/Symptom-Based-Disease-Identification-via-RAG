...
CMPE 255 Spring 2026 — San José State University
...
| Vineet Kumar | 019140433 | MedQuAD preprocessing, embedding backends (MiniLM + PubMedBERT), FAISS index, dense retriever, cross-encoder rerank, prompt template for the explainer |
| Aishwarya Madhave | 019129110 | Synonym dictionary, fusion reranker, alpha sweep, ablation harness, Streamlit UI, plots, paper writing lead |

All three authors are enrolled in CMPE 255 Spring 2026. No external collaborators contributed to this project.

## How we worked

We met twice a week (Tue + Sat). Sakshat owned the data side end-to-end, Vineet owned everything in the retrieval stack, and Aishwarya owned the fusion logic and the demo. The report and the IEEE-format slide deck were written collaboratively in Overleaf and Google Slides; the final reproducible PDFs in `/report` and `/slides` are generated from scripts in `code/scripts/` so anyone can rebuild them.
