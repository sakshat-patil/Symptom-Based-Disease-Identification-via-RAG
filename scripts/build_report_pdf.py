...
This is the primary deliverable PDF. The LaTeX source under report/ is the
canonical version (Overleaf-ready); this script produces a comparable PDF
without needing a full TeX Live install.
"""
...
# Per the team's submission convention, report lives outside the project
# folder so the grader can find it without digging through code.
OUT = ROOT.parent / "deliverables" / "report" / "final_report.pdf"
...
def two_col_template():
    ...
    body_h = title_y - MARGIN - 0.1 * inch  # explicit gap below title
...
def fig(path: Path, width: float, caption_text: str, styles) -> list:
...
def build():
    ...
    story = []
    # ---- Title block (spans both columns by living above the frames) ----
    story.append(Paragraph("Record-Based Medical Diagnostic Assistant: "
                            "A Hybrid FP-Growth + Retrieval-Augmented Pipeline",
                            s["title"]))
    story.append(Paragraph(
        "Sakshat Nandkumar Patil (SJSU 018318287) &nbsp;&middot;&nbsp; "
        "Vineet Kumar (SJSU 019140433) &nbsp;&middot;&nbsp; "
        "Aishwarya Madhave (SJSU 019129110)<br/>"
        "<i>Department of Computer Engineering, San Jos&eacute; State University, San Jos&eacute;, CA</i><br/>"
        "{sakshat.patil, vineet.kumar, aishwarya.madhave}@sjsu.edu<br/>"
        "<font size=8>CMPE 255 Data Mining (Spring 2026) &middot; All three authors are enrolled in the course; no external collaborators</font>",
        s["authors"]))
    story.append(FrameBreak())  # move to page1_left frame
    story.append(NextPageTemplate("rest"))  # subsequent pages use 2-col only
    ...
    story.append(Paragraph(
        ...
        "diseases to extract high-confidence {symptom set} -> disease "
        ...
        "(3072d) -- and bridges the training/retrieval vocabulary gap with "
        "a curated UMLS-style synonym dictionary. A linear fusion layer "
        "combines the two signals via FusedScore(d) = a&middot;RetrievalSim(d) "
        "+ (1-a)&middot;MiningConf(d). On 200 synthetic test cases, the best "
        "fused configuration (MiniLM + synonym expansion, a=0.3) achieves "
        "Recall@1 = 82.5%, Recall@10 = 89.5%, and MRR = 0.857, exceeding the "
        "mining-only baseline (Recall@1 = 79%) by 3.5 percentage points. The "
        "system ships as a three-tier application&mdash;a Next.js 14 web "
        "frontend, a FastAPI inference microservice, and a Pinecone-backed "
        "vector tier&mdash;that displays, for every ranked diagnosis, the "
        "matching FP-Growth rule, claim-level highlights from the retrieved "
        "MedQuAD passages, and a four-section structured clinical "
        "explanation, making every prediction auditable.",
        s["abstract"]))
    ...
    P("Diagnostic decision-making is hindered by the volume and "
      ...
      "query a literature database, and reconcile the two&mdash;a cognitive "
      "burden that black-box decision-support systems either replicate or "
      "hide behind opaque scores. We instead build an explicitly "
      "<i>interpretable</i> middle path: a system that mines human-readable "
      "{symptom}&#8594;disease rules from a patient transaction table while "
      "pairing every prediction with retrieved biomedical passages.")
    ...
    P("Concretely, this paper contributes (i) a fully reproducible "
      ...
      "dictionary; (iii) a four-way ablation over {encoder} &times; "
      ...
      "held-out cases; and (iv) a Next.js 14 web client backed by a FastAPI "
      "microservice that exposes every component live&mdash;a fusion-weight "
      "slider, a backend/explainer toggle, claim-level evidence highlights, "
      "an animated pipeline timeline, an LLM-backed clinical-gloss popover "
      "for individual symptom tokens, and a one-paragraph differential "
      "summary above the ranked diagnoses.")
    ...
    P("To honour Step 1 of the proposal end-to-end, "
      ...
      "Synthea's per-row label noise hurts mining recall, but the path is "
      "production-ready.")
    ...
    P("<b>Output:</b> 23,839 rules spanning all 41 disease labels; median "
      ...
      "{nodal_skin_eruptions} &#8594; fungal_infection (confidence 1.00, "
      "lift 41.0).")
    P("<b>Query-time scoring:</b> for input symptom set Q, "
      "MiningConf(Q,d) = max over (A&#8594;d) with A &#8838; Q of "
      "c(A&#8594;d) &middot; (|A &cap; Q| / |A|). The overlap weighting "
    ...
    P("All 24,063 passages are embedded by the chosen backend, "
      ...
      "3-large</font></b> (3072-dim, hits an OpenAI-compatible v1 "
      "endpoint via OPENAI_BASE_URL). The Azure backend produces the "
      "highest-quality vectors of the three on our cardiac probe (Heart "
      "Attack ranks #1 with retrieval_score 0.438 vs 0.000 with FAISS+"
    ...
    P("The reranker is intentionally simple: "
      "FusedScore(d) = a &middot; RetrievalSim(d) + (1-a) &middot; "
      ...
      "the order mining&rarr;retrieval&rarr;disease name. The default "
      "a=0.3 is chosen from the alpha sweep below.")
    ...
    P("Bi-encoders score query and passage independently. A cross-encoder "
      ...
      "(i) <b>Local cross-encoder</b> &mdash; the lightweight "
      "<font face='Courier' size=8>cross-encoder/ms-marco-MiniLM-L-6-v2"
      "</font> running on Apple MPS, ~100 ms per query; "
      "(ii) <b>Pinecone Inference</b> &mdash; a hosted reranker behind "
      "the same Pinecone API key, defaulting to <font face='Courier' size=8>"
      ...
      "Cohere isn't authorised. The FastAPI service auto-selects which "
    ...
    P("&bull; <b>Template explainer</b> &mdash; deterministic, "
      "citation-faithful. Stitches the matching FP-Growth rule and verbatim "
      "sentences from the top retrieved passages into the same four-section "
      "structure the LLM produces, each section tagged with its passage id. "
      "Always available. Used as the automatic fallback when the LLM call "
      "fails for any reason.")
    P("&bull; <b>OpenAI structured explainer</b> &mdash; activated when "
      ...
      "(link, prior, evidence-quality, what's-missing). Citations are "
    ...
    P("A 1,000-character paragraph is not 'grounded evidence' for a "
      ...
      "(NIH NHLBI/CDC = tier 1, etc.); (iii) a passage-type label "
      "(symptoms, diagnosis, treatment, ...) inferred from the question "
      "text; (iv) a specificity score = fraction of query symptoms "
      "actually mentioned in the passage. Cards are sorted by "
    ...
    P("<font face='Courier' size=8>src/vector_store.py</font> exposes a "
      ...
      "Azure-OpenAI embeddings. The architectural win for Pinecone in this "
      "project is metadata filtering -- a clinician toggle like 'only "
      ...
      "The API service guards against backend/index dimension mismatches "
      "with a clear 400 error, so the operator never sees a cryptic "
      "Pinecone 'dimension 768 does not match 3072' deep in the stack.")

    h("4.9 Test Suite")
    P("We ship <b>155 tests across 13 modules</b>, split between 147 unit "
      "tests (run in ~4s with no network) and 8 live integration tests "
      "gated on env vars (<font face='Courier' size=8>pytest -m live</font>). "
      "Coverage spans token normalisation, FP-Growth scoring, fusion "
      "arithmetic, evidence extraction, clinical-explanation schema, the "
      "FAISS metadata filter, the Azure OpenAI embedder (with a stub "
      "client for unit tests + real-endpoint live test), the OpenAI "
      "explainer (mocked + live), and the Pinecone reranker (auto-fallback "
      "from Cohere to BGE on 403). The suite includes regression tests "
      "for three real bugs caught during development: the short-keyword "
      "false-positive (<font face='Courier' size=8>hav</font> matching "
      "inside <font face='Courier' size=8>have</font>), a plural form "
      "missed by the passage-type regex "
      "(<font face='Courier' size=8>treatments</font>), and a "
      "dimension-mismatch crash when Pinecone (3072d) was queried with "
      "PubMedBERT (768d) embeddings.")
    ...
    P("The system is split into three tiers: a Next.js 14 web UI "
      ...
      "(Aishwarya = web, Vineet = AI service, Sakshat = data and ML "
      "pipeline), gives us deployable boundaries (the AI service can run "
      "on a GPU host, the UI on a CDN), and matches how a real "
      "decision-support tool would be deployed.")
    P("The choice is a UI toggle. Both modes guarantee that every claim "
      "made in the explanation is traceable to a retrieved passage id, "
    ...
    P("<b>Test cases:</b> 200 synthetic queries drawn from the transaction "
      "table with seed 42. Each picks a random true disease, samples 2-5 of "
      "its canonical symptoms uniformly, and adds a noise symptom with "
      "probability 0.2.")
    ...
    story.append(make_table([
        ...
        ["PubMedBERT+syn", "fused 0.3", "0.815", "0.875", "0.875", "0.843"],
                       ...
                       0.45 * inch, 0.4 * inch]))
    story.append(Paragraph("Table 3: Ablation on 200 test cases. Best fused "
                             "row in bold rationale follows.", s["caption"]))
    P("Three observations: (i) <b>mining alone is a strong baseline</b> "
      ...
      "but complementary</b>&mdash;even the best retrieval-only "
      "configuration tops out at 26.5% R@10, confirming the vocabulary "
      "gap; (iii) <b>fusion is monotone-improving</b>: every retrieval "
      "variant fused with mining at a=0.3 exceeds mining alone on R@1. "
      "MiniLM with synonym expansion is the strongest fused variant, "
      "suggesting that within this synthetic distribution lexical "
      "bridging matters more than encoder pre-training.")

    if (RESULTS / "alpha_sweep.png").exists():
        story += fig(RESULTS / "alpha_sweep.png", 3.1 * inch,
                      "Figure 1: Fusion sensitivity to a (PubMedBERT + "
    ...
    P("Retrieval-only mode tops out near 26.5% R@10 even with the best "
      "configuration. Two causes: (i) MedQuAD indexes ~600 disease concepts "
      "but only ~30 of our 41 Kaggle classes have a dedicated focus "
      "article (Diabetes, Asthma, ...), so eight diseases inevitably fall "
      "back to near-miss passages; (ii) the focus-only matching policy "
      "that suppresses false positives like 'What is Chest Pain?' being "
      "attributed to every cardiopulmonary disease trades retrieval recall "
      "for precision. The fusion layer hides both issues whenever mining "
      "finds a rule, which is why a=0.3 works well in practice.")

    h("6.5 System Latency")
    P("Step 6 of the proposal calls out system latency as an evaluation "
      "metric. We instrumented every pipeline stage and ran 100 queries on "
    ...
    P("The full RAG path with PubMedBERT, cross-encoder, and the OpenAI "
      ...
      "queries); the cross-encoder adds ~100 ms and the LLM adds ~500 ms. "
      "The template explainer keeps the default path well under the "
      "1 s/query target.")
    ...
    P("The system ships as a Next.js 14 web client (<font face='Courier' "
      ...
      "(<font face='Courier' size=8>service/api.py</font>). The left "
      "rail exposes encoder backend, mode, an alpha slider, synonym "
      "expansion, cross-encoder rerank, and explainer choice (template "
      "vs. OpenAI gpt-4o-mini). The main column shows, for every ranked "
      "diagnosis, a fused-score progress bar with mining and retrieval "
      "sub-scores, the matching FP-Growth rules with confidence and lift, "
      "the four-section clinical explanation, and expandable evidence "
      "cards with claim-level sentence highlights and source authority "
      "tier badges.")
    P("Beyond the core pipeline display, three lightweight AI-assist "
      ...
      "seven server-side stages (encode, vector search, attribution, "
      "mining, fusion, evidence assembly, explanation) with running "
      "estimated milliseconds while the request is in flight, then snaps "
      "to real per-stage latencies once the response lands. (2) A "
      "<i>differential summary</i> endpoint synthesises a one-paragraph "
      "clinician-style note across the top three ranked candidates "
    ...
    P("<b>Synthetic evaluation.</b> All 200 test cases come from the same "
      ...
      "be noisier. The mining component is the most exposed&mdash;synonym "
      "expansion already insulates retrieval from this distribution shift.")
    P("<b>Linear fusion.</b> The fusion equation is a one-parameter convex "
      "combination. A small learned reranker (XGBoost or a "
      "&lt;disease, passage&gt; cross-encoder) using rule lift, passage "
      "provenance, and symptom overlap is the natural next step.")
    P("<b>Disease universe.</b> The keyword index assumes a closed universe "
      "of 41 labels. Extending to open-domain diagnoses requires either "
      "automatic disease canonicalisation against UMLS, or treating "
      "fusion as a passage-level rather than disease-level operation.")
    ...
    P("<b>Sakshat Nandkumar Patil</b> (SJSU 018318287) &mdash; data ETL "
    ...
    P("<b>Vineet Kumar</b> (SJSU 019140433) &mdash; MedQuAD preprocessing; "
      ...
      "Inference reranker (Cohere/BGE) with auto-fallback; FastAPI "
      "inference service (REST schema, lifespan model loading, CORS, "
      "/diagnose endpoint, dimension-mismatch guard); OpenAI structured-"
      "output prompt for the clinical explainer.")
    P("<b>Aishwarya Madhave</b> (SJSU 019129110) &mdash; curated 130-entry "
      ...
      "authority tiers and passage-type classification; Next.js 14 web UI "
      "(symptom picker, diagnosis cards, evidence highlighting, source "
      "filters, latency strip, three-backend selector); 155-test pytest "
      "suite including stub-client unit tests and live Azure+Pinecone "
